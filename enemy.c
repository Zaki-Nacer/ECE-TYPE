...
void set_enemy_state(GameState *gameState, Ennemi *e, EnemyState new_state) {
    ...
    if (e->current_sprites == NULL) {
    ...
    } else if (e->current_nb_frames <= 0) {
        printf("ERREUR CRITIQUE: Ennemi %ld (type %d), état %d: current_nb_frames est %d!\n", enemy_index, e->type, new_state, e->current_nb_frames); fflush(stdout);
        e->active = 0; // Désactiver par sécurité
        ...
        // printf("DEBUG: Ennemi %ld (type %d), état %d: Sprites assignés OK (w=%d, h=%d).\n", enemy_index, e->type, new_state, e->w, e->h); fflush(stdout);
    }
...
void dessiner_ennemis(GameState *gameState) {
    ...
    for (int i = 0; i < MAX_ENNEMIS; i++) {
        ...
        if (e->active) {
            ...
            // *** Log si l'ennemi est actif mais n'a pas été dessiné (car hors écran ou erreur sprite) ***
            // if (!dessine) {
            //    printf("DEBUG: Ennemi %d actif mais non dessiné (screen_x=%d, visible=%d, sprites_ok=%d)\n",
            //           i, screen_x, (screen_x + e->w > 0 && screen_x < gameState->screen_width_allegro),
            //           (e->current_sprites && e->imgcourante >= 0 && e->imgcourante < e->current_nb_frames && e->current_sprites[e->imgcourante] != NULL));
            //    fflush(stdout);
            // }
...
void nettoyer_ressources_ennemis(GameState *gameState) {
    ...
    printf("Sprites modèles ennemis détruits.\n"); fflush(stdout);
}
